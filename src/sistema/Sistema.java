...
public class Sistema implements ISistema {
    ...
    public void modificarCliente(String nickname, String nombre, String email, String contrasenia, String urlImagen,
        ...
        Cliente cliente = new Cliente(nickname.trim(), nombre.trim(), email.trim(), contrasenia.trim(), urlImagen != null ? urlImagen.trim() : null,
    ...
    public DtUsuario altaAerolinea(String nickname, String nombre, String email, String contrasenia, String urlImagen,
        ...
        Aerolinea aerolinea = new Aerolinea(nickname.trim(), nombre.trim(), email.trim(), pass.trim(), urlImagen != null ? urlImagen.trim() : null,
    ...
    public Cliente obtenerClientePorNickname(String nickname) {return clienteDAO.obtenerClientePorNickname(nickname); }
    public Aerolinea obtenerAerolineaPorNick(String nickAerolinea) {return aerolineaDAO.obtenerAerolineaPorNick(nickAerolinea); }
    // ===================== RUTAS Y VUELOS =====================
   ...
   /* public List<String> listarNicknamesClientes() {
    ...
    public void reservarVuelo(String nickCliente, String nombreVuelo, TipoAsiento tipoAsiento,
        ...
        if (nombresPasajeros.size() != cantPasajes || apellidosPasajeros.size() != cantPasajes)
            throw new IllegalArgumentException("Los nombres y apellidos de pasajeros no coinciden con la cantidad de pasajes");
        ...
        if (vuelo == null) throw new IllegalArgumentException("Vuelo no encontrado");

        // Verificar disponibilidad
        Long ocupados = vueloDAO.contarAsientosOcupados(nombreVuelo, tipoAsiento);
        int maxAsientos = (tipoAsiento == TipoAsiento.TURISTA) ? vuelo.getAsientosTurista() : vuelo.getAsientosEjecutivo();
    ...
    // ===================== ALTA PAQUETES =====================
    public void altaPaqueteRV(String nombre, String descripcion, Integer diasValidez, Float descuento, LocalDate fecha) {
    ...
    public void agregarRutaPaquete(String nombrePaquete, String nickAerolinea, String nombreRuta, Integer cantidad, TipoAsiento tipoAsiento) {
        if (nombrePaquete == null) throw new IllegalArgumentException("Debe indicar un Paquete de Vuelo");
        if (nickAerolinea == null) throw new IllegalArgumentException("Debe indicar una Aerolinea");
        if (nombreRuta == null) throw new IllegalArgumentException("Debe indicar una Ruta de Vuelo");
        if (tipoAsiento == null) throw new IllegalArgumentException("Debe indicar el tipo de asiento");
        if (cantidad == null || cantidad <= 0) throw new IllegalArgumentException("La cantidad debe ser mayor a 0");
    ...
    private boolean esEmailValido(String email) {
        ...
        return !email2.isEmpty() && email2.contains("@") && email2.indexOf('@') > 0 && email2.indexOf('@') < email2.length() - 1;
    }

    public boolean validarCedulaUruguaya(String cedula) {
        cedula = cedula.replace(".", "").replace("-", "");
        if (!cedula.matches("\\d{7,8}")) return false;
